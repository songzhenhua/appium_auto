...
class BasePage(object):
    ...
    def split_locator(self, locator):
        ...
        # desc是Appium中的find_element_by_accessibility_id()，对应元素content-desc属性
        locator_dict = {
            'id': 'id',
            'name': 'name',
            'class': 'class name',
        ...
        if by not in locator_dict.keys():
            raise NameError("wrong locator!'id','name','class','tag','link','plink','xpath','css','desc',exp:'id,username'")
    ...
    def wait_element(self, locator, sec=30):
        ...
        try:
            if by == 'desc':
                WebDriverWait(self.driver, sec, 1).until(lambda x: x.find_element_by_accessibility_id(value),
    ...
    def screenshot(self, info='-'):
        ...
        log.info(u'screenshot：%s.png' % info)
    ...
    def switch_to_context(self, context):
        ...
        log.info(u'切换context至：%s' % context)
